...
public final class Library {
    ...
    /**
     * 默认的库名 tcnative-1.dll libtcnative-1.dll
     */
    ...
    private Library() throws Exception {
        ...
        for (int i = 0; i < NAMES.length; i++) {//遍历默认的库名
            try {
                //加载库默认的库
                ...
                throw t;
            } catch (Throwable t) {//加载默认的库名失败  比如加载 tcnative-1
                //获取tcnative-1.dll
                String name = System.mapLibraryName(NAMES[i]);
                for (int j = 0; j < paths.length; j++) {
                    //遍历所有的系统库 实例化库路径下的文件对象 比如d:\maven-3.5.2\bin\tcnative-1.dll
                    java.io.File fd = new java.io.File(paths[j] , name);
                    if (fd.exists()) {//如果文件存在 说明存在这个dll文件 但是加载失败  直接抛出异常
